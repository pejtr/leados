 ...
 *  - OPTIHUB internal (Stripe orders stored in DB, if any)
 *
...
export const globalEarningsRouter = router({
  ...
  summary: protectedProcedure.query(async () => {
    ...
    const projectCount = dsrHeaders() ? 2 : 1; // DSR + OPTIHUB
    return {
  ...
  snapshot: protectedProcedure.query(async (): Promise<GlobalEarningsResult> => {
    ...
    // ── OPTIHUB (placeholder — Stripe revenue from DB can be added here) ──────
    projects.push({
      id: "leadOS",
      name: "OPTIHUB",
