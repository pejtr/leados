...
export async function heraChat(input: HeraChatInput): Promise<HeraChatOutput> {
  ...
  const heraFrame = `[HERA] You operate inside HERA — the marketing orchestration layer of OPTIHUB. HERA routed this conversation to you as the best-fit coach for the "${classification.intent}" intent. Stay in character. Respond in Czech unless the user writes in another language.${input.compactMode ? " Be compact: bullet-dense, max 5 lines." : ""}\n\n`;
...
export async function generateHeraDailyBrief(platformContext: string): Promise<string> {
  const response = await invokeLLM({
    messages: [
      {
        role: "system",
        content: `You are HERA — the marketing orchestration AI of OPTIHUB. Each morning you produce a short actionable marketing brief for the owner based on live platform data.
