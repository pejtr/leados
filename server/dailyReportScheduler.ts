...
async function generateAISummary(stats: DailyStats): Promise<string> {
  try {
    const prompt = `You are a concise business analyst. Based on these 24h OPTIHUB metrics, write a 2-3 sentence executive summary in Czech language. Be specific about numbers and give one actionable insight.
...
<div class="container">
  ...
  <div class="footer">
    OPTIHUB · crmleadsystem.com · Automatický denní report<br>
    ...
    if (ENV.forgeApiUrl && ENV.forgeApiKey) {
      try {
        ...
        const res = await fetch(emailEndpoint, {
          ...
          body: JSON.stringify({
            to: config.email,
            subject: `OPTIHUB Denní Report — ${stats.totalSales} prodejů, $${stats.totalRevenue.toFixed(0)} revenue`,
            html: emailHtml,
          }),
        });
    ...
    if (!emailSent) {
      const notifContent = `📊 Denní Report OPTIHUB\n\n` +
      ...
      await notifyOwner({
        title: `OPTIHUB Report — $${stats.totalRevenue.toFixed(0)} revenue dnes`,
