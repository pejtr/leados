# DEPLOY — OPTIMATEO do produkce

Cíl: dostat web živě na `www.optimateo.com`, aby Sklik reklama → `/web` → poptávka →
...
## 4. Doména
1. V DNS u `www.optimateo.com` nastav CNAME/A záznam na Railway target.
2. V Railway → Settings → Domains přidej `www.optimateo.com`.
3. Počkej na SSL (automaticky).

## 5. Smoke test (než pustíš reklamu)
- [ ] `https://www.optimateo.com/` se načte
- [ ] `https://www.optimateo.com/web` se načte (Sklik landing)
...
2. Kampaň → cílová URL `https://www.optimateo.com/web`.
