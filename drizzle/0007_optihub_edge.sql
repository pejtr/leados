...
-- Versioned, repeatable (IF NOT EXISTS), reviewable. Applied by drizzle-kit migrate.
--
-- Time columns are epoch milliseconds (bigint) so behaviour does not depend on the
-- database/session timezone. Raw credential secrets are never stored: only the
-- SHA-256 hex hash.
