...
import {
  ...
  CheckCircle, XCircle, AlertTriangle, Info, Loader2, Clock, History, FileText, Printer, TrendingUp
} from "lucide-react";
...
export default function WebAudit() {
  ...
  const convertToLeadMutation = trpc.webAudit.convertToLead.useMutation({
    onSuccess: (data) => {
      toast.success("Web uložen jako Lead v CRM!");
        ...
        {result && (
          <div className="space-y-4">
            ...
            <div className="bg-card border border-border rounded-xl p-5">
              ...
              {/* Quick checks */}
              <div className="grid grid-cols-2 md:grid-cols-5 gap-2 mt-5 print:hidden">
                {[
            ...
            {/* Action bar for CRM / PDF */}
            <div className="flex items-center justify-end gap-3 print:hidden">
              <Button onClick={() => window.print()} variant="outline" className="border-border text-foreground hover:bg-muted">
                <Printer className="w-4 h-4 mr-2" /> Stáhnout PDF report
              </Button>
              {result.id && (
                <Button
