...
import { Plus, Trash2, Mail, ChevronRight, Users, Edit2, Save } from "lucide-react";
...
export default function Sequences() {
  ...
  const [steps, setSteps] = useState<Array<{ stepNumber: number; delayDays: number; subject: string; body: string }>>([]);
  ...
  const saveStepsMut = trpc.sequences.saveSteps.useMutation({
    onSuccess: () => {
      ...
      toast.success("Steps saved: Email sequence steps updated.");
    },
  });
  ...
  function startEditing() {
    setSteps(stepsData.length > 0 ? stepsData.map(s => ({ stepNumber: s.stepNumber, delayDays: s.delayDays, subject: s.subject, body: s.body })) : [
  ...
  function addStep() {
    ...
    setSteps([...steps, { stepNumber: steps.length + 1, delayDays: (last?.delayDays ?? 0) + 3, subject: "", body: "" }]);
  }
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-white/50 text-sm mt-1">Automated multi-step outreach campaigns for your leads</p>
        </div>
              ...
              <div className="space-y-3 mb-6">
                ...
                {stepsData.length === 0 && (
                  <p className="text-white/30 text-sm py-4 text-center border border-dashed border-white/10 rounded-lg">
                    No steps yet — click "Edit Steps" to add email steps
                  </p>
                )}
                {stepsData.map((step, i) => (
                  <div key={step.id} className="flex gap-3 p-3 bg-white/5 rounded-lg border border-white/10">
                    <div className="w-8 h-8 rounded-full bg-emerald-500/20 flex items-center justify-center text-sm flex-shrink-0">
                      {STEP_ICONS[i] ?? "📧"}
                    </div>
          ...
          <div className="space-y-4">
            {steps.map((step, i) => (
              <div key={i} className="p-4 border border-white/10 rounded-lg space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-semibold text-emerald-400">Step {step.stepNumber}</span>
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2">
                      <label className="text-xs text-white/50">Send on day</label>
                ...
                <Input value={step.subject} onChange={e => setSteps(steps.map((s, j) => j === i ? { ...s, subject: e.target.value } : s))} placeholder="Email subject..." className="bg-white/5 border-white/20 text-white text-sm" />
                <Textarea value={step.body} onChange={e => setSteps(steps.map((s, j) => j === i ? { ...s, body: e.target.value } : s))} placeholder="Email body... Use {{firstName}}, {{companyName}}, {{industry}}" className="bg-white/5 border-white/20 text-white text-sm resize-none" rows={4} />
              </div>
            ))}
            <Button variant="outline" onClick={addStep} className="w-full border-dashed border-white/20 text-white/50 hover:border-emerald-500/50 hover:text-emerald-400">
